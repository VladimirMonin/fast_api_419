...
async def create_order(
    ...
    5. Очищаем корзину
    6. Коммитим всё в одной транзакции
    ...
    """
    # Шаг 1: Получаем корзину с товарами
    ...
    logger.info(f"📦 Создание заказа для пользователя {user_id}. Товаров в корзине: {len(cart.items)}")
...
async def get_order_by_id(
    ...
    if order:
        ...
        logger.warning(f"⚠️ Заказ #{order_id} не найден или недоступен для пользователя {user_id}")
